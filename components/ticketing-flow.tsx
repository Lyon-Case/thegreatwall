...
export function TicketingFlow() {
  ...
  return <div className="ticket-flow">
    ...
    {step === 3 && <div className="confirmation"><div className="confirmation-icon"><Check size={34} /></div><SectionTag>03 / Signal confirmed</SectionTag><h2>You&apos;re on the <em>list.</em></h2><p>Your access pass for <strong>{event.name}</strong> is reserved. We&apos;ve sent the confirmation to {guest.email}.</p><div className="pass-card"><Ticket size={22} /><div><span>{event.date}</span><strong>{tier.name}</strong><small>{guest.name} · {quantity} pass{quantity > 1 ? 'es' : ''}</small></div><b>#{confirmationCode}</b></div><a className="button button-cyan" href="/">Return to command center <span>↗</span></a></div>}
  </div>
}
